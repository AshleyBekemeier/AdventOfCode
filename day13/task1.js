...
for (let i = 0; i < blocks.length; i++) {
  ...
  while (hMatches) {
    hDuplicateIndex = lines.findIndex(
      (line, index, self) => line === self[index + 1] && index > lastIndex
    );
  ...
  if (horizontalMirror) {
    ...
    while (vMatches) {
      ...
      for (let j = lastIndex + 1; j < lines[0].length - 1; j++) {
        const charsLeft = lines.map((line) => line[j]);
        const charsRight = lines.map((line) => line[j + 1]);

        if (charsLeft.every((char, pos) => char === charsRight[pos])) {
